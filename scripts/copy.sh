...
if grep -iq '^\(ID_LIKE\|ID\)=.*\(debian\|ubuntu\)' /etc/os-release >/dev/null 2>&1; then
    ...
    print_color $WARNING "
    █▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀█
                 Qompass DOTS version INCOMPATIBLE
    █▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄█

    Debian / Ubuntu detected. Refer to Qompass Dotfiles README
    For instruction on how to update your Qompass Dotfiles
...
while true; do
    echo "${NOTE} ${SKY_BLUE} By default, Qompass AI Dots are configured for 1440p or 2k."
...
while true; do
    echo -e "${NOTE} ${SKY_BLUE} By default, Qompass AI's Dots are configured in 24H clock format."
...
if [ -d "$BACKUP_DIR_PATH" ]; then
    ...
    "
    echo -e "${MAGENTA}Kindly Visit and check Qompass AI Dotfile GitHub page for the history of commits.${RESET}"
