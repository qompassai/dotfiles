{
  ...
  outputs = inputs@{ self, flake-parts, nixpkgs, nixpkgs-unstable, qjp, flake-schemas, nix-on-droid, ... }:
    let
      flakePartsOutputs = flake-parts.lib.mkFlake { inherit inputs; } (
        let
          ...
          jetsonConfigs = nixpkgs.lib.mapAttrs (n: cfg: mkJetsonConfig ({ name = n; } // cfg)) hardwareConfigs;
        in {
          ...
          perSystem = { system, pkgs, ... }:
            let
              ...
              mkBasePackages = with pkgs; [
                blender btop cava chezmoi cmake conda curl direnv docker dunst
                easyeffects firejail fish fuzzel fzf gpg gh git jq khal libreoffice lua
                maven mypaint neomutt neovim nix nixpkgs-fmt nodejs nvfetcher nvimpager
                pgadmin4 pip pipewire pnpm postgresql python3 python3Packages.pytest qt5ct qt6ct R
                rage sass sops sqlite steam tor-browser-bundle-bin unbound waybar wget
                wireguard-tools wofi zig zls unstable.nil
              ];
            in {
            ...
            default = {
              ...
              specialArgs = {
                ...
                androidPackages = with nixpkgs.legacyPackages.aarch64-linux; [
                  btop curl git jq neomutt nixpkgs-fmt python3 sops wget wireguard-tools zls
                ];
              };
            };
          };
