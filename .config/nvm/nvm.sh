    ...
    if [ $NVM_PATTERN_STARTS_WITH_V = true ] && [ "_$(nvm_num_version_groups "${PATTERN}")" = "_3" ]; then
      ...
      *)
      ...
      if [ -n "${NVM_DIRS_TO_SEARCH1}${NVM_DIRS_TO_SEARCH2}${NVM_DIRS_TO_SEARCH3}" ]; then
        VERSIONS="$(
          command find "${NVM_DIRS_TO_SEARCH1}"/* "${NVM_DIRS_TO_SEARCH2}"/* "${NVM_DIRS_TO_SEARCH3}"/* -name . -o -type d -prune -o -path "${PATTERN}*" |
            command sed -e "
...
system"
  ...
  nvm_ls_remote_index_tab() {
    ...
    *)
    ...
    local VERSION_LIST
    VERSION_LIST="$(
      nvm_download -L -s "${MIRROR}/index.tab" -o - |
        command sed "
        1d;
        s/^/${PREFIX}/;
      "
...
BEGIN {
  ...
  nvm_get_arch() {
    ...
    *) NVM_ARCH="${HOST_ARCH}" ;;
    ...
    if [ "$(uname)" = "Linux" ] &&
      [ "${NVM_ARCH}" = arm64 ] &&
      [ "$(command od -An -t x1 -j 4 -N 1 "/sbin/init" 2>/dev/null)" = ' 01' ]; then
      NVM_ARCH=armv7l
      HOST_ARCH=armv7l
    fi
...
' # necessary to turn off variable expansion except for newlines
          for LINK in ${LINKS}; do
            set +f
            unset IFS # restore variable expansion
            if [ -n "${LINK}" ]; then
              case "${LINK}" in
