# /qompassai/dotfiles/fish/conf.d/autopair.fish
# Qompass AI Fish Autopair Config
...
function _autopair_fish_key_bindings --on-variable fish_key_bindings
    set --query fish_key_bindings[2] >/dev/null; or return
    test $fish_key_bindings = fish_default_key_bindings &&
        set --local mode default insert ||
        set --local mode insert default
    bind --mode $mode[-1] --erase \177 \b \t
    bind --mode $mode[2] \177 _autopair_backspace
    bind --mode $mode[2] \b _autopair_backspace
    bind --mode $mode[2] \t _autopair_tab
    printf "%s\n" $autopair_pairs | while read --local left right --delimiter ""
        bind --mode $mode[-1] --erase $left $right
        if test $left = $right --quiet
            bind --mode $mode[2] $left "_autopair_insert_same \\$left"
        else
            bind --mode $mode[2] $left "_autopair_insert_left \\$left \\$right"
            bind --mode $mode[2] $right "_autopair_insert_right \\$right"
        end
    end
end
