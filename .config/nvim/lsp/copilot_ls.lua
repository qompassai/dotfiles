...
local function sign_in(bufnr, client)
    client:request(
        ...
        function(err, result)
            ...
            if result.command then ---@cast result { userCode: string, command: lsp.Command }
                ...
                if continue == 1 then
                    client:exec_cmd(command, { bufnr = bufnr }, function(cmd_err, cmd_result)
                        if cmd_err then
                            vim.echo(cmd_err.message, vim.log.levels.ERROR)
                            return
                        end
                        if cmd_result.status == 'OK' then
                            vim.echo('Signed in as ' .. cmd_result.user .. '.')
                        end
                    end)
                end
            end
            if result.status == 'PromptUserDeviceFlow' then
                vim.echo('Enter your one-time code ' .. result.userCode .. ' in ' .. result.verificationUri)
            elseif result.status == 'AlreadySignedIn' then
                vim.echo('Already signed in as ' .. result.user .. '.')
            end
        end
    )
end
