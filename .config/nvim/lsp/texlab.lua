...
vim.lsp.config['texlab'] = {
  ...
  on_attach = function(client, bufnr)
      ...
      return function()
        return client:exec_cmd(
          ...
          function(err, _)
            if err then
              vim.notify(
                ('Failed to clean %s files: %s'):format(kind, err.message),
    ...
    local cmds = {
      ...
      { name = 'TexlabFindEnvironments',  fn = buf_find_envs,                desc = 'Find environments at current position' },
      { name = 'TexlabChangeEnvironment', fn = buf_change_env,               desc = 'Change environment at current position' },
    }
