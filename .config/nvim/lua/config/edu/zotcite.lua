...
local function exists(p) return p and vim.uv.fs_stat(p) ~= nil end
...
local function xdg_runtime() return os.getenv("XDG_RUNTIME_DIR") or vim.uv.os_tmpdir() end
local cfg = {
...
if cfg.tmpdir and not exists(cfg.tmpdir) then vim.uv.fs_mkdir(cfg.tmpdir, 448) end
