...
function M.setup_cicdmap()
  ...
  map(
    ...
    "<leader>zli",
    "<cmd>:Lzi<CR>",
    vim.tbl_extend("force", opts, { desc = "Zoxide interactive local to window" })
  )
  -- In normal mode, press 'Space' + 'z' + 'l' + 'i' for interactive window-local navigation

  -- Zoxide: Interactive local to tab
  map("n", "<leader>zti", "<cmd>:Tzi<CR>", vim.tbl_extend("force", opts, { desc = "Zoxide interactive local to tab" }))
