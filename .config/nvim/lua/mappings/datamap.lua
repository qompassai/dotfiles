...
function M.setup_datamap()
  ...
  -- Show signature help
  map("n", "<leader>nsh", vim.lsp.buf.signature_help, vim.tbl_extend("force", bufopts, { desc = "[n]vim-lsp [s]how [s]ignature help" }))
  ...
  -- Rename symbol
  map("n", "<leader>nrs", vim.lsp.buf.rename, vim.tbl_extend("force", bufopts, { desc = "[n]vim-LSP [r]ename [s]ymbol" }))
  ...
  -- Show references
  map("n", "nsr", vim.lsp.buf.references, vim.tbl_extend("force", bufopts, { desc = "[n]vim-LSP [s]how [r]eferences" }))
