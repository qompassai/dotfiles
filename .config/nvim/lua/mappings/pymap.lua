  ...
  vim.api.nvim_create_autocmd('LspAttach',
    {
      callback = function(ev)
        ...
        map('n', '<leader>jrc', function()
          ...
          {
            desc = '[j]upyter [r]evaluate [c]ell'
          }
        ))
        ...
        map(
          ...
          vim.tbl_extend('force', opts, { desc = '[j]upyter [s]tart [k]ernel' })
        )
        ...
        map(
          ...
          vim.tbl_extend('force', opts, { desc = '🦦 [j]upter [o]utput toggle panel' })
        )
        ...
        map(
          ...
          vim.tbl_extend('force', opts, { desc = '[j]upyter [t]erminal [m]ark' })
        )
        ...
        map(
          ...
          vim.tbl_extend('force', opts, { desc = '[j]upyter [t]erminal [s]et' })
        )

        -- UV.nvim Keymaps

        map('n', '<leader>ur', '<cmd>UVRunFile<CR>', vim.tbl_extend('force', opts, { desc = 'UV Run File' }))
        map(
          'v',
          '<leader>us',
        ...
        map('n', '<leader>ui', '<cmd>UVInit<CR>', vim.tbl_extend('force', opts,
          {
            desc = 'UV Initialize'
          }
        ))
        map('n', '<leader>ua', '<cmd>UVAdd<CR>', vim.tbl_extend('force', opts, {
          desc = 'UV Add Package'
        }
        ))
        map('n', '<leader>ud', '<cmd>UVRemove<CR>', vim.tbl_extend('force', opts, {
          desc = 'UV Remove Package'
        }
        ))
        map('n', '<leader>uc', '<cmd>UVSync<CR>', vim.tbl_extend('force', opts, {
