...
local function setup_aimap()
    ...
    map('n', '<C-g>c', ':RoseChatNew<CR>',
        vim.tbl_extend('force', opts, {desc = '[r]ose [c]hat'}))
    -- In normal mode, press 'Ctrl' + 'g' + 'c' to open a new chat.

    -- Rose Toggle Popup Chat
    map('n', '<C-g>t', ':RoseChatToggle<CR>',
        vim.tbl_extend('force', opts, {desc = '[r] Toggle Popup Chat'}))
    -- In normal mode, press 'Ctrl' + 'g' + 't' to toggle the popup chat.

    -- Rose Chat Finder
    map('n', '<C-g>f', ':RoseChatFinder<CR>',
        vim.tbl_extend('force', opts, {desc = 'Rose Chat Finder'}))
    ...
    map('n', '<C-g>r', ':RoseRetry<CR>',
        vim.tbl_extend('force', opts, {desc = 'Rose Repeat last action'}))
...
local function setup_dbee_mappings()
    ...
    local function dbstore(data_type, store_type, args)
        ...
        if not status then
            vim.notify(
                'DBee: No active results to store. Please run a query first.',
    ...
    map('n', '<C-d>c', function() dbstore('csv', 'buffer', {extra_arg = 0}) end,
        vim.tbl_extend('force', opts,
                       {desc = '[D]bee Store CSV in [c]urrent buffer'}))

    -- Dbee Store results as JSON to file
    map('n', '<C-d>j', function()
        dbstore('json', 'file',
                {from = 2, to = 7, extra_arg = 'path/to/file.json'})
    end, vim.tbl_extend('force', opts,
                        {desc = '[D]bee Store JSON to [j]son file'}))

    -- Dbee Yank results as table
    map('n', '<C-d>y',
        function() dbstore('table', 'yank', {from = 0, to = 1}) end,
        vim.tbl_extend('force', opts, {desc = '[D]bee [y]ank results as table'}))

    -- Dbee Yank last 2 rows as CSV
    map('n', '<C-d>r',
        function() dbstore('csv', 'yank', {from = -3, to = -1}) end,
        vim.tbl_extend('force', opts, {desc = '[D]bee yank [r]ows as CSV'}))

    -- Dbee Run query
    map('n', '<C-d>q', ':DBee query<CR>',
        vim.tbl_extend('force', opts, {desc = '[D]bee [q]uery'}))
end
