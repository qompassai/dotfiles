...
function M.setup_genmap()
  ...
  vim.api.nvim_create_autocmd('LspAttach', {
    callback = function(ev)
      ...
      map('i', '<C-e>', '<End>', vim.tbl_extend('force', opts,
        {
          desc = 'Move to the end of the line' }))

      -- Move left by one character while in insert mode
      map('i', '<C-h>', '<Left>', vim.tbl_extend('force', opts,
        {
      ...
      map('n', '<Esc>', '<cmd>noh<CR>', vim.tbl_extend('force',
        opts,
        {
      ...
      map('n', '<C-j>', '<C-w>j', vim.tbl_extend('force', opts,
        {
      ...
      map('n', '<C-k>', '<C-w>k', vim.tbl_extend('force', opts,
        {
      ...
      map('n', '<C-s>', '<cmd>w<CR>', vim.tbl_extend('force', opts,
        {
      ...
      map('n', '<leader>F', function()
        if #vim.lsp.get_clients(
          ...
          vim.echo('No active LSP client with formatting support.',
            vim.log.levels.WARN)
        end
      end, {
