...
find_stored_default_node_hook = SimpleEventHook({
    ...
    execute = function(event)
        local source = event:get_source() ---added
        local props = source:get_properties() ---changed from local props = event:get_properties()
        ...
        for _, node_props in ipairs(available_nodes) do --- Check if any of the available nodes matches any of the configured
...
store_configured_default_nodes_hook = SimpleEventHook({
    ...
    execute = function(event)
        local source = event:get_source() ---added
        local props = source:get_properties() --- changed from local props = event:get_properties()
        local def_node_type = props['event.subject.key']:sub(20) --- get the part after "default.configured." (= 19 chars)
...
metadata_added_hook = SimpleEventHook({ --- set initial values
...
---@return string[] stored --- Collect all the previously configured node names from the state file
function collectStored(def_node_type) ---@param def_node_type string
    local stored = {}
...
---@param def_node_type string --- Store the given node names in the state file
function updateStored(def_node_type, stored) ---@param stored string[]
    local key_base = 'default.configured.' .. def_node_type
    local key = key_base
    local index = 0
