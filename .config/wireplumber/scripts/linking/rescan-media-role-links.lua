...
function setVolume(om, link, level)
    ...
    local media_role_other_lnkbl = om:lookup({
        ...
        Constraint({
            'node.link-group',
            '=',
            media_role_lnkbl.properties['node.link-group'],
        }),
    ...
    if media_role_other_lnkbl then
        ...
        if n then
            log:info(
                string.format(
                    '.. %s volume of media role node "%s(%d)" to %f',
                    level < 1.0 and 'duck' or 'restore',
    ...
    steps = {
        start = {
            ...
            execute = function(event, transition)
                local source, om, _, si_props, _, _ = lutils:unwrap_select_target_event(event)
                ...
                do
                    ...
                    local active = (bit.band(link:get_active_features(), Feature.SessionItem.ACTIVE) ~= 0)
                    log:debug(
                        string.format(
                            ' .. looking at link(%d) active %s pmrl %s',
                            link.id,
                            tostring(active),
                            tostring(link == pmrl)
                        )
