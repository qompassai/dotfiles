...
SimpleEventHook({
    ...
    execute = function(event)
        ...
        do
            ...
            log:debug(
                string.format('Looking at: %s (%s)', tostring(target_props['node.name']), tostring(target_node_id))
            )
            ...
            local passthrough_compatible, can_passthrough = lutils.checkPassthroughCompatibility(si, target)
