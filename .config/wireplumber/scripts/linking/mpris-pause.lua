-- /qompassai/dotfiles/.config/wireplumber/scripts/linking/mpris-pause.lua
-- Qompass AI WirePlumber MPRIS-Pause Linking Script
    ...
    execute = function(event)
        ...
        for client_id in pairs(client_ids) do
            ...
            log:debug(
                si,
                string.format(
                    'node %s removed: was linked to client %s (%s pid %s)',
            ...
            for _, player in ipairs(players) do
                ...
                elseif client.properties['pipewire.access.portal.app_id'] == player['flatpak-app-id'] then
                    ...
                    if instance_id == nil then
                        ...
                        log:debug(
                            si,
                            string.format('.. match pipewire.access.portal.app_id = %s', player['flatpak-app-id'])
                        )
                        match = true
                    elseif instance_id == player['flatpak-instance-id'] then
                        log:debug(
                            si,
                            string.format(
                                '.. match pipewire.access.portal.app_id = %s instance_id = %s',
                                player['flatpak-app-id'],
                                player['flatpak-instance-id']
                            )
                        )
                        match = true
                    end
        ...
        for bus_name in pairs(pause_players) do
            log:info(
                si,
                string.format(
                    'node %s removed: pausing linked media player %s',
                    tostring(si.properties['node.id']),
                    bus_name
                )
            )
