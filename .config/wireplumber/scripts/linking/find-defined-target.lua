-- /qompassai/dotfiles/.config/wireplumber/scripts/linking/find-defined-target.lua
-- Qompass AI WirePlumber Find-Defined-Target Linking Script
...
SimpleEventHook({
    ...
    execute = function(event)
        ...
        local metadata = Settings.get_boolean('linking.allow-moving-streams') and cutils.get_default_metadata_object()
        ...
        elseif target_value then
            for lnkbl in om:iterate({ type = 'SiLinkable' }) do
                ...
                if
                    (target_props['node.name'] == target_value or target_props['object.path'] == target_value)
                    and target_props['item.node.direction'] == cutils.getTargetDirection(si_props)
                    and lutils.canLink(si_props, lnkbl)
                then
                    target_picked = true
                    target = lnkbl
                    break
        ...
        if target_picked and target then
            log:info(
                si,
                string.format(
                    '... defined target picked: %s (%s), can_passthrough:%s',
