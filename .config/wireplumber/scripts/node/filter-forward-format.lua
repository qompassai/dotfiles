...
function onLinkGroupPortsStateChanged(si, old_state, new_state)
    ...
    if old_state == 'configured' then
        log:debug(si, 'ports already configured, ignoring transition on ' .. si_props['node.name']) --- Already configured -> configured again; ignore
        return
    end
...
SimpleEventHook({
    ...
    execute = function(event)
        ...
        if Settings.get_boolean('node.filter.forward-format') then --- Forward filters ports format to associated virtual devices if enabled
            local si_props = si.properties
            local link_group = si_props['node.link-group']
            local si_flags = lutils:get_flags(si.id)
            if --- only listen for ports state changed on audio filter streams
