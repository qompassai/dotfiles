-- /qompassai/dotfiles/.config/wireplumber/scripts/device/state-routes.lua
-- Qompass AI Wireplumber Best Profile Finder Device Lua Script
    ...
    execute = function(event)
        ...
        for _, device_id in ipairs(active_ids) do
            ...
            for _, ri in pairs(dev_info.route_infos) do
                if
                    cutils.arrayContains(ri.devices, tonumber(device_id))
                    and (ri.profiles == nil or cutils.arrayContains(ri.profiles, dev_info.active_profile))
                    and cutils.arrayContains(spr, ri.name)
                then
                    route_info = ri
                    break
                end
            ...
            if route_info then
                selected_routes[tostring(device_id)] = Json.Object({ index = route_info.index }):to_string()
            end
            ::next_device_id::
        end
    ...
    execute = function(event)
        ...
        for p in device:iterate_params('Route') do
            ...
            if not route_info.prev_active then
                ...
                log:info(device, string.format('new active route(%s) found of device(%s)', route.name, dev_info.name))
                route_info.prev_active = true
                route_info.active = true

                selected_routes[tostring(route.device)] = Json.Object({ index = route_info.index }):to_string()
                push_select_routes = true
            elseif route.save and route.props then
                -- just save route properties
                log:info(device, string.format('storing route(%s) props of device(%s)', route.name, dev_info.name))
