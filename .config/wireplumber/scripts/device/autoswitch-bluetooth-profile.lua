-- /qompassai/dotfiles/.config/wireplumber/scripts/device/autoswitch-bluetooth-profile.lua
-- Qompass AI Wireplumber AutoSwitch Bluetooth Profile Device Lua Script
-- Copyright (C) 2025 Qompass AI, All rights reserved
------------------------------------------------------------------------------------------
utils = require('linking-utils')
...
function findProfile(device, index, name)
    for p in device:iterate_params('EnumProfile') do
        ...
        if profile ~= nil then
            if (index ~= nil and profile.index == index) or (name ~= nil and profile.name == name) then
                return profile
            end
        end
    end
...
function switchDeviceToHeadsetProfile(dev_id, device_om) ---@param device_om WPObjectManager
    ...
    if profile ~= nil then
        ...
        log:info(device, 'Switching profile from: ' .. cur_profile.name .. ' to: ' .. profile.name)
...
function restoreProfile(dev_id, device_om)
    ...
    if cur_profile ~= nil and not hasProfileInputRoute(device, cur_profile.index) then
        log:info(device, 'Current profile is already a non-headset profile, no need to restore')
        return
    end
    ...
    if profile ~= nil then
        ...
        log:info(device, 'Restoring profile from: ' .. cur_profile.name .. ' to: ' .. profile.name)
        device:set_params('Profile', pod)
    else
...
---@param dev_id integer|string
---@param device_om WPObjectManager
...
function getLinkedBluetoothLoopbackSourceNodeForStream(stream, node_om, link_om, visited_link_groups)
    ...
    if bt_node ~= nil then
        ...
        if filter_main_node ~= nil then
            ...
            do
                local bt_node = getLinkedBluetoothLoopbackSourceNodeForStream(
                    filter_stream_node,
                    node_om,
                    link_om,
                    visited_link_groups
...
local evaluate_bluetooth_profiles_hook = SimpleEventHook({
    ...
    execute = function(event)
        ...
        do
            ...
            if
                bt_node_state == 'running'
                and isBluetoothLoopbackSourceNodeLinkedToStream(bt_node, node_om, link_om)
            then
