-- /qompassai/dotfiles/.config/wireplumber/scripts/device/select-routes.lua
-- Qompass AI Wireplumber Select Routes Device Lua Script
    ...
    execute = function(event)
        ...
        for p in device:iterate_params('EnumRoute') do
            ...
            if route_info.available ~= route.available then
                log:info(
                    device,
                    'route ' .. route.name .. ' available changed ' .. route_info.available .. ' -> ' .. route.available
                )
        ...
        if profile then
            ...
            if profile_changed or avail_routes_changed then
                log:info(
                    device,
                    string.format('restore routes for profile(%s) of device(%s)', profile.name, dev_info.name)
                )
        ...
        for _, p in ipairs(profile.classes) do
            if type(p) == 'table' and p.pod_type == 'Struct' then
                ...
                while true do
                    ...
                    if k == 'card.profile.devices' and type(v) == 'table' and v.pod_type == 'Array' then
