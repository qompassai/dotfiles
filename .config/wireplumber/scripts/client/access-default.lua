...
---@return WPAccess|nil
function getAccess(properties) ---@param properties WPProperties
...
---@return WPPermissions|nil perms
---@return WPAccess|nil access
...
function getPermissions(properties)
    if config.rules then
        local mprops, matched = JsonUtils.match_rules_update_properties(config.rules, properties)
...
clients_om:connect('object-added', function(om, client)
    ...
    if perms ~= nil then
        log:info(client, 'Granting permissions to client ' .. id .. ' (access ' .. effective_access .. '): ' .. perms)
