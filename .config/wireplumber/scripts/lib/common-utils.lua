...
function cutils.mediaClassToDirection(media_class)
    if media_class:find('Sink') or media_class:find('Input') or media_class:find('Duplex') then
        return 'input'
...
function cutils.getTargetDirection(properties)
    ...
    if
        ...
        or (properties['item.node.direction'] == 'input' and cutils.parseBool(properties['stream.capture.sink']))
    then
...
function cutils.getDefaultNode(properties, target_direction)
    local target_media_class = properties['media.type'] .. (target_direction == 'input' and '/Sink' or '/Source')
...
function cutils.get_object_manager(name)
    cutils.source_plugin = cutils.source_plugin or Plugin.find('standard-event-source')
    cutils.object_managers[name] = cutils.object_managers[name] or cutils.source_plugin:call('get-object-manager', name)
