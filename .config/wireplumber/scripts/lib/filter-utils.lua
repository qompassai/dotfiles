...
local function getFilterSmart(metadata, node)
    if metadata ~= nil then --- Check metadata
    ...
    local prop_str = node.properties['filter.smart'] --- Check node properties
    if prop_str ~= nil then
        return cutils.parseBool(prop_str)
    end
    return false --- Otherwise consider the filter not smart by default
end
local function getFilterSmartName(metadata, node)
    ...
    local prop_str = node.properties['filter.smart.name'] --- Check node properties
    if prop_str ~= nil then
        return prop_str
    end
    return node.properties['node.link-group'] --- Otherwise use link group as name
end
local function getFilterSmartDisabled(metadata, node)
    ...
    local prop_str = node.properties['filter.smart.disabled'] --- Check node properties
    if prop_str ~= nil then
        return cutils.parseBool(prop_str)
    end
    return false --- Otherwise consider the filter not disabled by default
end
local function getFilterSmartTargetable(metadata, node)
    ...
    local prop_str = node.properties['filter.smart.targetable'] --- Check node properties
    if prop_str ~= nil then
        return cutils.parseBool(prop_str)
    end
    return false --- Otherwise consider the filter not targetable by default
end
local function getFilterSmartTarget(metadata, node, om)
    ...
    do
        ...
        if n_target.properties['node.link-group'] ~= nil and getFilterSmart(metadata, n_target) then
            goto continue
        end
...
local function getFilterSmartBefore(metadata, node) -- Check metadata and fallback to properties
    local id = node['bound-id']
    local value_str = nil
    if metadata ~= nil then
    ...
    local before_json = Json.Raw(value_str) --- Parse
...
local function getFilterSmartAfter(metadata, node)
    local id = node['bound-id'] --- Check metadata and fallback to properties
    local value_str = nil
    if metadata ~= nil then
...
local function insertFilterSorted(curr_filters, filter)
    ...
    for i, v in ipairs(before_filters) do --- Add the filters to the new table stored
...
local function rescanFilters(om, metadata_om)
    local metadata = metadata_om:lookup({ Constraint({
        'metadata.name',
        '=',
        'filters',
    }) })
    ...
    for si in om:iterate({ type = 'SiLinkable' }) do
        ...
        -- Filter direction
        if string.find(filter.media_class, 'Audio/Sink') or string.find(filter.media_class, 'Video/Sink') then
            filter.direction = 'input'
        else
            filter.direction = 'output'
        end
        filter.media_type = si.properties['media.type'] --- Filter media type
        filter.smart = getFilterSmart(metadata, n) --- Get filter properties
...
function module.get_filter_target(direction, link_group)
    ...
    local index = nil
    for i, v in ipairs(module.filters) do
        if v.direction == direction and v.link_group == link_group and not v.disabled and v.smart then
            filter = v
            index = i
            break
        end
    ...
    for i, v in ipairs(module.filters) do -- Return the next filter with matching target
        if
            v.direction == direction
            and v.media_type == filter.media_type
            and v.name ~= filter.name
            and v.link_group ~= link_group
            and not v.disabled
            and v.smart
            and ((v.target == nil and filter.target == nil) or (v.target ~= nil and filter.target ~= nil and v.target.id == filter.target.id))
            and i > index
        then
            return v.main_si
        end
...
function module.get_filter_from_target(direction, media_type, si_target)
    ...
    if si_target then
        ...
        if target_link_group ~= nil then
            ...
            for i, v in ipairs(module.filters) do
                if
                    v.direction == direction
                    and v.media_type == media_type
                    and v.link_group == target_link_group
                    and not v.disabled
                    and v.smart
                then
                    filter = v
    ...
    for i, v in ipairs(module.filters) do -- Find the first filter matching target
        if
            v.direction == direction
            and v.media_type == media_type
            and not v.disabled
            and v.smart
            and ((v.target ~= nil and target ~= nil and v.target.id == target.id) or (target == nil and v.targetless))
        then
            return v.main_si
        end
    end
...
--[[ --- changed 96-143 to fix code break
local function getFilterSmartTarget(metadata, node, om)
  ...
  for si_target in om:iterate { type = "SiLinkable" } do
    ...
    for key, val in pairs(match_rules) do --- Make sure the target node properties match all rules
      if n_target.properties[key] ~= tostring(val) then
        goto skip_target
      end
    end
