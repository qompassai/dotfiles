...
function lutils.clearPriorityMediaRoleLink(link) --- if the link happens to be priority one, clear it and find the next priority.
    local lprops = link.properties
    local lmc = lprops['target.media.class']
    pmrl = lutils.getPriorityMediaRoleLink(lmc)
    if pmrl ~= link then --- only proceed if the link happens to be priority one.
        return
    end
    local prio_link = nil
    local prio = 0
    local plugged = 0
    for l in
        cutils.get_object_manager('session-item'):iterate({
            type = 'SiLink',
            Constraint({
                'item.factory.name',
                '=',
    ...
    do
        ...
        if pmrl == l then -- dont consider this link as it is about to be removed.
            goto continue
        end
        if getprio(link) > prio or (getprio(link) == prio and getplugged(link) > plugged) then
            prio = getprio(l)
...
function lutils.updatePriorityMediaRoleLink(link) --- record priority media role link
    local lprops = link.properties
    ...
    if getprio(link) > getprio(pmrl) or (getprio(link) == getprio(pmrl) and getplugged(link) >= getplugged(pmrl)) then
        setPriorityMediaRoleLink(mc, link)
    end
end
...
---@param lmc string
---@param link WPObject|nil
function setPriorityMediaRoleLink(lmc, link)
    lutils.priority_media_role_link[lmc] = link
    if link then
        Log.debug(
            string.format(
                'update priority link(%d) media role("%s") priority(%d)',
                link.id,
                link.properties['media.role'],
                getprio(link)
            )
        )
    else
...
function lutils.is_role_policy_target(si_props, target_props)
    return Core.test_feature('hooks.linking.role-based.rescan') -- role-based policy links are those that link to targets with
        and cutils.parseBool(target_props['policy.role-based.target']) -- policy.role-based.target = true, unless the stream is a monitor
        and not cutils.parseBool(si_props['stream.monitor']) -- (usually pavucontrol) or the stream is linking to the monitor ports
        and si_props['item.node.direction'] ~= target_props['item.node.direction'] -- of a sink (both are "input")
end
...
function lutils.canPassthrough(si, si_target)
    ...
    if --- both nodes must support encoded formats
        not cutils.parseBool(props['item.node.supports-encoded-fmts'])
        or not cutils.parseBool(tprops['item.node.supports-encoded-fmts'])
    then
        return false
    end
    local n1 = si:get_associated_proxy('node') --- make sure that the nodes have at least one common non-raw format
...
function lutils.checkFollowDefault(si, si_target)
    ...
    if reconnect and not is_filter then
        local def_id = cutils.getDefaultNode(si_props, cutils.getTargetDirection(si_props))
...
function lutils.canLink(properties, si_target)
    ...
    local function isMonitor(properties)
        return properties['item.node.direction'] == 'input'
            and cutils.parseBool(properties['item.features.monitor'])
            and not cutils.parseBool(properties['item.features.no-dsp'])
            and properties['item.factory.name'] == 'si-audio-adapter'
    end
    ...
    if properties['item.node.direction'] == target_props['item.node.direction'] and not isMonitor(target_props) then
        return false
    end

    local function canLinkGroupCheck(link_group, si_target, hops) -- check link group
        local target_props = si_target.properties
...
function lutils.checkPassthroughCompatibility(si, si_target)
    ...
    local si_target_must_passthrough = cutils.parseBool(si_target.properties['item.node.encoded-only'])
    local can_passthrough = lutils.canPassthrough(si, si_target)
    if (si_must_passthrough or si_target_must_passthrough) and not can_passthrough then
...
function lutils.haveAvailableRoutes(si_props, devices_om)
    ...
    local found = 0 --- Check if available routes support the node's card.profile.device
    ...
    if found == 0 then -- The node is part of a profile without routes so we assume it
        return true -- is available. This can happen for Pro Audio profiles
    end
    return false
end
