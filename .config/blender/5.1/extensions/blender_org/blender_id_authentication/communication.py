...
def blender_id_session():
    ...
    # Load the manifest file to get the add-on version
    SCRIPT_DIR = path.dirname(__file__)
    manifest_file = path.join(SCRIPT_DIR, "blender_manifest.toml")
    ...
    try:
        ...
        return dict(
            ...
            error_message=format('There was a problem setting up a connection to '
                                 'the server. Error type is: %s' % type(e).__package__)
        )
