...
class Calc:
    ...
    def eval_input(self):
        try:
            ...
            namespace['self'] = CalcData(self.ans_flo, self.org, self.input, self.unit_system, self.unit_length, self.rna_unit)
            interpreter.env = namespace
            v = interpreter.visit(parse(f'_out = {self.input}'))['_out']
            if isinstance(v, int):
                try: float(v)
                except:
