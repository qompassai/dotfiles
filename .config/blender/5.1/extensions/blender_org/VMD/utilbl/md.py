...
def tr_action_fcurves(oj):
    try:    return r_fcurves(oj.animation_data)
    except: return None
    #|
def tr_dr_add(oj, dp):
...
def copy_kf(fc_from, tar, tar_dp): # fc_from != None
    ...
    if tar.animation_data:
        ...
        elif r_fcurves(tar.animation_data) is None:     is_empty = True
    else: is_empty = True

    if is_empty:
        if tar.keyframe_insert(data_path = tar_dp) == False: return
        fcs     = r_fcurves(tar.animation_data)
...
def copy_md_keyframe(oj, oj_tar, tar_md, dpDot, dpDot_tar):
    try: fcurves = r_fcurves(oj.animation_data)
    except: return
...
def r_code_copy_to_selected(
                            ...
                            modifiers):
    if use_code:
        s = f'# The following Python code can be overridden\n# Restricted Python {rpy.RpyInterpreter.VERSION_STR}\n\n'
    else:
        s = f'# Enable "Override Code" to run Python Code\n# Restricted Python {rpy.RpyInterpreter.VERSION_STR}\n\n'
...
class ModRefAttr:
    ...
    BOOLEAN = ('object',)
    BEVEL = ('vertex_group', 'edge_weight', 'vertex_weight',)
    BUILD = ()
    ...
    GREASE_PENCIL_TEXTURE = ('tree_node_filter', 'material_filter')
    GREASE_PENCIL_TIME = ('frame_end', 'frame_start', 'tree_node_filter')
    GREASE_PENCIL_VERTEX_WEIGHT_PROXIMITY = (
        'tree_node_filter',
        'material_filter',
        'object',
    ...
    GREASE_PENCIL_BUILD = (
        ...
        'object',
        'target_vertex_group',)
    GREASE_PENCIL_DASH = ('tree_node_filter', 'material_filter')
    GREASE_PENCIL_ENVELOPE = (
        'tree_node_filter',
        'material_filter',
        'vertex_group_name',)
    GREASE_PENCIL_LENGTH = ('tree_node_filter', 'material_filter')
    GREASE_PENCIL_MIRROR = ('tree_node_filter', 'material_filter', 'object')
    GREASE_PENCIL_MULTIPLY = ('tree_node_filter', 'material_filter')
    GREASE_PENCIL_OUTLINE = (
        'tree_node_filter',
        'material_filter',
        'object',
        'outline_material',)
    GREASE_PENCIL_SIMPLIFY = ('tree_node_filter', 'material_filter')
    GREASE_PENCIL_SUBDIV = ('tree_node_filter', 'material_filter')
    LINEART = (
    ...
    GREASE_PENCIL_THICKNESS = (
        ...
        'material_filter',
        'vertex_group_name',)
    GREASE_PENCIL_COLOR = ('tree_node_filter', 'material_filter')
    GREASE_PENCIL_TINT = (
        'tree_node_filter',
        'material_filter',
        'object',
