...
class AreaBlockTabModifierEditor(AreaBlockTab):
    ...
    def r_object_layers(self):
        ...
        return []
        #|
    def r_layer_groups(self):
        ob = self.w.active_object
        if hasattr(ob, "data") and hasattr(ob.data, "layer_groups") and ob.data.layer_groups:
            return ob.data.layer_groups
        return []
        #|
    ...
    def init_tab_CLOTH(self):
        ...
        button_search.fn = self.r_bufn_search(upd_data_callback, [
            ...
            search_confirm_callback = search_confirm_callback)
        self.upd_data_callback = upd_data_callback
        #|
            ...
            if self.w.active_modifier.canvas_settings is None:
                ...
                def fn_darklight_settings(surf):
                    if ui_state_format == [surf.surface_format, surf.surface_type, surf.init_color_type, surf.is_cache_user]: pass
                        ...
                        if surface_format == "VERTEX":
                            ps_surface["image_resolution"].dark()
                        else:
                            ps_surface["image_resolution"].light()
                        return True
    ...
    def init_tab_FLUID(self):
        ...
        if _fluid_type == "DOMAIN":
            ...
            b_export.items[-1].rna = RNA_openvdb_data_depth
            b_export.items[-1].enum_items = RNA_openvdb_data_depth.enum_items
            ...
            def fn_darklight(dom):
                ...
                if dom.domain_type == "GAS":
                    ...
                    if dom.is_cache_baking_any:
                        ...
                        if dom.sndparticle_combined_export == 'OFF':
                            ps["use_spray_particles"].light()
                        else:
                            ps["use_spray_particles"].dark()

                        if dom.has_cache_baked_particles or not (dom.use_spray_particles or dom.use_foam_particles or dom.use_bubble_particles):
    ...
    def init_tab_MESH_SEQUENCE_CACHE(self):
        ...
        o_read_data = b0.r_prop("read_data", text="Read", options={"ROW_LENGTH": 3})
        ...
        if self.w.active_modifier.cache_file:
            ...
            def fn_darklight_cache(cac):
                ...
                if ui_state == [uianim.library_state, cac.override_frame, cac.is_sequence]: return
                ui_state[:] = [uianim.library_state, cac.override_frame, cac.is_sequence]


                if cac.is_sequence:
                    ps["frame_offset"].dark()
                else:
                    ps["frame_offset"].light()
    ...
    def init_tab_MULTIRES(self):
        ...
        o_reshape_conform = b2.function(RNA_MULTIRES_conform_base, self.bufn_MULTIRES_conform_base, isdarkhard=True)
        b2.sep(2)
        b2.prop("filepath")
        props["filepath"].set_align("FULL")
        b2.sep(2)
    ...
    def init_tab_REMESH(self):
        ...
        def fn_darklight(md):
            if ui_state == [ui_anim_data.library_state, md.mode, md.use_remove_disconnected]: return
            ui_state[:] = [ui_anim_data.library_state, md.mode, md.use_remove_disconnected]
    ...
    def init_tab_SOLIDIFY(self):
        ...
        def fn_darklight(md):
            ...
            if md.solidify_mode == 'NON_MANIFOLD':
                ...
                props["nonmanifold_merge_threshold"].light()
            else:
                props["use_flat_faces"].dark()
                ...
                props["nonmanifold_merge_threshold"].dark()
    ...
    def init_tab_SUBSURF(self):
        ...
        if (ob.cycles and bpy.context.engine == 'CYCLES' and md == ob.modifiers[-1]):
            ...
            b1 = ui.new_block(title=ui.r_prop("use_adaptive_subdivision", text="Adaptive Subdivision", options={"HEAD"}))
            b1.prop_flag("adaptive_space")
            props["adaptive_space"].r_button_width = self.r_button_width_150
            b1.prop("adaptive_pixel_size")
            b1.prop("adaptive_object_edge_length")

            ui_anim_cycles = b1.set_pp(lambda: self.w.active_object.cycles, ob.cycles, lambda: "cycles")
            ui_properties = Info1(self)

            ui_anim_data_properties = b1.set_pp(lambda: ui_properties, ui_properties, NS)
            o_info = b1.r_prop("info", text="Final Scale")
            o_info.set_align("FULL")
            b_scene = b1.new_block(title=o_info)
            ...
            b1.sep(1)
            b1.set_pp_from(ui_anim_data)

            # <<< 1copy (0definit_tab_SUBSURF_advanced,, $$)
            ...
            def fn_darklight(md):
                ...
                if ui_state == [ui_anim_data.library_state, md.use_limit_surface, md.use_adaptive_subdivision, md.adaptive_pixel_size, md.adaptive_object_edge_length, scene_cycles.dicing_rate, scene_cycles.preview_dicing_rate, md.adaptive_space]: return
                ui_state[:] = [ui_anim_data.library_state, md.use_limit_surface, md.use_adaptive_subdivision, md.adaptive_pixel_size, md.adaptive_object_edge_length, scene_cycles.dicing_rate, scene_cycles.preview_dicing_rate, md.adaptive_space]

                if ui_anim_data.library_state == 1: return

                if md.use_adaptive_subdivision:
                    props["adaptive_space"].light()
                    if md.adaptive_space == 'PIXEL':
                        fac = md.adaptive_pixel_size
                        props["adaptive_pixel_size"].light()
                        props["adaptive_object_edge_length"].dark()
                    elif md.adaptive_space == "OBJECT":
                        fac = md.adaptive_object_edge_length
                        props["adaptive_pixel_size"].dark()
                        props["adaptive_object_edge_length"].light()
                    else:
                        fac = md.adaptive_pixel_size
                        props["adaptive_pixel_size"].dark()
                        props["adaptive_object_edge_length"].dark()

                    ui_properties.info = f"Render {max(scene_cycles.dicing_rate * fac, 0.1):.2f} px  |  Viewport {max(scene_cycles.preview_dicing_rate * fac, 0.1):.2f} px"
                    o_info.upd_data()
                    o_info.light()
            ...
            def upd_data_callback():
                md = self.w.active_modifier
                ob = self.w.active_object
                if (ob.cycles and bpy.context.engine == 'CYCLES' and md == ob.modifiers[-1]): pass
                else:
                    self.init_tab(self.active_tab, push=False, evtkill=False)
                    return
    ...
    def init_tab_WAVE(self):
        ...
        def fn_darklight(md):
            ...
            if md.use_normal:
                props["use_normal_x"].light()
            else:
                props["use_normal_x"].dark()
    ...
    def init_tab_GREASE_PENCIL_TEXTURE(self):
        ...
        b_influence.prop('use_layer_group_filter')
        b_influence.prop_inv_search("invert_layer_filter", "tree_node_filter", GpuImg_OUTLINER_DATA_GP_LAYER, self.r_object_layers)
        props["tree_node_filter"].r_button_width = self.r_button_width_166
        ...
        props["material_filter"].r_except_objects = lambda: {e for e in bpy.data.materials if not e.is_grease_pencil}
        ...
        def fn_darklight(md):
            if ui_state == [ui_anim_data.library_state, md.mode, md.use_layer_pass_filter, md.use_material_pass_filter, md.use_layer_group_filter]: return
            ui_state[:] = [ui_anim_data.library_state, md.mode, md.use_layer_pass_filter, md.use_material_pass_filter, md.use_layer_group_filter]
            ...
            if md.use_layer_group_filter:
                if props["tree_node_filter"].enum_items != self.r_layer_groups:
                    props["tree_node_filter"].enum_items = self.r_layer_groups
                    props["tree_node_filter"].update_icon()
                    props["tree_node_filter"].set_title('Group', b_influence.w.box_block.L)
            else:
                if props["tree_node_filter"].enum_items != self.r_object_layers:
                    props["tree_node_filter"].enum_items = self.r_object_layers
                    props["tree_node_filter"].update_icon()
                    props["tree_node_filter"].set_title('Layer', b_influence.w.box_block.L)
    ...
    def init_tab_GREASE_PENCIL_TIME(self):
        ...
        if self.w.active_modifier.mode == "CHAIN":
            ...
            def fn_darklight(md):
                if ui_state == [ui_anim_data.library_state, md.use_layer_pass_filter, md.use_layer_group_filter]: return
                ui_state[:] = [ui_anim_data.library_state, md.use_layer_pass_filter, md.use_layer_group_filter]
            ...
            def fn_darklight(md):
                if ui_state == [ui_anim_data.library_state, md.mode, md.use_custom_frame_range, md.use_layer_pass_filter, md.use_layer_group_filter]: return
                ui_state[:] = [ui_anim_data.library_state, md.mode, md.use_custom_frame_range, md.use_layer_pass_filter, md.use_layer_group_filter]
    ...
    def init_tab_GREASE_PENCIL_VERTEX_WEIGHT_PROXIMITY(self):
        ...
        def fn_darklight(md):
            if ui_state == [ui_anim_data.library_state, md.target_vertex_group, md.vertex_group_name, md.use_layer_pass_filter, md.use_material_pass_filter, md.use_layer_group_filter]: return
            ui_state[:] = [ui_anim_data.library_state, md.target_vertex_group, md.vertex_group_name, md.use_layer_pass_filter, md.use_material_pass_filter, md.use_layer_group_filter]
    ...
    def init_tab_GREASE_PENCIL_ARRAY(self):
        ...
        def fn_darklight(md):
            if ui_state == [ui_anim_data.library_state, md.use_relative_offset, md.use_constant_offset, md.use_object_offset, md.use_layer_pass_filter, md.use_material_pass_filter, md.use_layer_group_filter]: return
            ui_state[:] = [ui_anim_data.library_state, md.use_relative_offset, md.use_constant_offset, md.use_object_offset, md.use_layer_pass_filter, md.use_material_pass_filter, md.use_layer_group_filter]
    ...
    def init_tab_GREASE_PENCIL_BUILD(self):
        ...
        def fn_darklight(md):
            if ui_state == [ui_anim_data.library_state, md.mode, md.time_mode, md.use_restrict_frame_range, md.use_fading, md.use_layer_pass_filter, md.use_material_pass_filter, md.use_layer_group_filter]: return
            ui_state[:] = [ui_anim_data.library_state, md.mode, md.time_mode, md.use_restrict_frame_range, md.use_fading, md.use_layer_pass_filter, md.use_material_pass_filter, md.use_layer_group_filter]
    ...
    def init_tab_GREASE_PENCIL_DASH(self):
        ...
        def fn_darklight(md):
            if ui_state == [ui_anim_data.library_state, md.use_layer_pass_filter, md.use_material_pass_filter, md.use_layer_group_filter]: return
            ui_state[:] = [ui_anim_data.library_state, md.use_layer_pass_filter, md.use_material_pass_filter, md.use_layer_group_filter]
    ...
    def init_tab_GREASE_PENCIL_ENVELOPE(self):
        ...
        def fn_darklight(md):
            if ui_state == [ui_anim_data.library_state, md.mode, md.vertex_group_name, md.use_layer_pass_filter, md.use_material_pass_filter, md.use_layer_group_filter]: return
            ui_state[:] = [ui_anim_data.library_state, md.mode, md.vertex_group_name, md.use_layer_pass_filter, md.use_material_pass_filter, md.use_layer_group_filter]
    ...
    def init_tab_GREASE_PENCIL_LENGTH(self):
        ...
        def fn_darklight(md):
            if ui_state == [ui_anim_data.library_state, md.use_random, md.use_curvature, md.mode, md.use_layer_pass_filter, md.use_material_pass_filter, md.use_layer_group_filter]: return
            ui_state[:] = [ui_anim_data.library_state, md.use_random, md.use_curvature, md.mode, md.use_layer_pass_filter, md.use_material_pass_filter, md.use_layer_group_filter]
    ...
    def init_tab_GREASE_PENCIL_MULTIPLY(self):
        ...
        def fn_darklight(md):
            if ui_state == [ui_anim_data.library_state, md.use_fade, md.use_layer_pass_filter, md.use_material_pass_filter, md.use_layer_group_filter]: return
            ui_state[:] = [ui_anim_data.library_state, md.use_fade, md.use_layer_pass_filter, md.use_material_pass_filter, md.use_layer_group_filter]
    ...
    def init_tab_GREASE_PENCIL_OUTLINE(self):
        ...
        props["outline_material"].r_except_objects = lambda: {e for e in bpy.data.materials if not e.is_grease_pencil}
        b0.prop("object", options={"ID":"OBJECT"})
        label0 = b0.label([""])
        ...
        def fn_darklight(md):
            if ui_state == [ui_anim_data.library_state, (True  if bpy.context.scene.camera else False), md.use_layer_pass_filter, md.use_material_pass_filter, md.use_layer_group_filter]: return
            ui_state[:] = [ui_anim_data.library_state, (True  if bpy.context.scene.camera else False), md.use_layer_pass_filter, md.use_material_pass_filter, md.use_layer_group_filter]
    ...
    def init_tab_LINEART(self):
        ...
        props["target_material"].r_except_objects = lambda: {e for e in bpy.data.materials if not e.is_grease_pencil}
        b0.sep(1)
        b0.prop("radius", text="Line Radius")
        b0.prop("opacity")
        ...
        def fn_darklight(md):
            ...
            if ui_state == [ui_anim_data.library_state, is_first_lineart_in_stack, md.is_baked, md.use_cache, md.source_type, (True  if md.light_contour_object else False), md.use_contour, md.use_custom_camera, md.use_face_mark, self.w.active_object.show_in_front, md.use_multiple_levels, md.level_end, md.use_material_mask, md.level_start, md.use_intersection, md.silhouette_filtering, md.use_geometry_space_chain]: return
            ui_state[:] = [ui_anim_data.library_state, is_first_lineart_in_stack, md.is_baked, md.use_cache, md.source_type, (True  if md.light_contour_object else False), md.use_contour, md.use_custom_camera, md.use_face_mark, self.w.active_object.show_in_front, md.use_multiple_levels, md.level_end, md.use_material_mask, md.level_start, md.use_intersection, md.silhouette_filtering, md.use_geometry_space_chain]
            ...
            if md.use_contour:
                ...
                if md.silhouette_filtering in {"GROUP", "INDIVIDUAL"}:
                    props["use_invert_silhouette"].light()
                else:
                    props["use_invert_silhouette"].dark()
            else:
            ...
            if md.use_geometry_space_chain:
                props["chaining_image_threshold"].set_text('Geometry Threshold')
            else:
                props["chaining_image_threshold"].set_text('Image Threshold')
    ...
    def init_tab_GREASE_PENCIL_HOOK(self):
        ...
        def fn_darklight(md):
            ...
            if ui_state == [ui_anim_data.library_state, state_object, name_object, md.falloff_type, md.vertex_group_name, md.use_layer_pass_filter, md.use_material_pass_filter, md.use_layer_group_filter]: return
            ui_state[:] = [ui_anim_data.library_state, state_object, name_object, md.falloff_type, md.vertex_group_name, md.use_layer_pass_filter, md.use_material_pass_filter, md.use_layer_group_filter]
    ...
    def init_tab_GREASE_PENCIL_LATTICE(self):
        ...
        def fn_darklight(md):
            if ui_state == [ui_anim_data.library_state, md.vertex_group_name, md.use_layer_pass_filter, md.use_material_pass_filter, md.use_layer_group_filter]: return
            ui_state[:] = [ui_anim_data.library_state, md.vertex_group_name, md.use_layer_pass_filter, md.use_material_pass_filter, md.use_layer_group_filter]
    ...
    def init_tab_GREASE_PENCIL_NOISE(self):
        ...
        def fn_darklight(md):
            if ui_state == [ui_anim_data.library_state, md.use_random, md.random_mode, md.vertex_group_name, md.use_layer_pass_filter, md.use_material_pass_filter, md.use_custom_curve, md.use_layer_group_filter]: return
            ui_state[:] = [ui_anim_data.library_state, md.use_random, md.random_mode, md.vertex_group_name, md.use_layer_pass_filter, md.use_material_pass_filter, md.use_custom_curve, md.use_layer_group_filter]
    ...
    def init_tab_GREASE_PENCIL_OFFSET(self):
        ...
        def fn_darklight(md):
            if ui_state == [ui_anim_data.library_state, md.offset_mode, md.vertex_group_name, md.use_layer_pass_filter, md.use_material_pass_filter, md.use_layer_group_filter]: return
            ui_state[:] = [ui_anim_data.library_state, md.offset_mode, md.vertex_group_name, md.use_layer_pass_filter, md.use_material_pass_filter, md.use_layer_group_filter]
    ...
    def init_tab_GREASE_PENCIL_SHRINKWRAP(self):
        ...
        b0.prop("smooth_step", text="Repeat")
        ...
        def fn_darklight(md):
            if ui_state == [ui_anim_data.library_state, md.wrap_method, md.use_negative_direction, md.cull_face, md.vertex_group_name, md.use_layer_pass_filter, md.use_material_pass_filter, md.use_layer_group_filter]: return
            ui_state[:] = [ui_anim_data.library_state, md.wrap_method, md.use_negative_direction, md.cull_face, md.vertex_group_name, md.use_layer_pass_filter, md.use_material_pass_filter, md.use_layer_group_filter]
    ...
    def init_tab_GREASE_PENCIL_SMOOTH(self):
        ...
        def fn_darklight(md):
            if ui_state == [ui_anim_data.library_state, md.use_edit_position, md.vertex_group_name, md.use_layer_pass_filter, md.use_material_pass_filter, md.use_layer_group_filter]: return
            ui_state[:] = [ui_anim_data.library_state, md.use_edit_position, md.vertex_group_name, md.use_layer_pass_filter, md.use_material_pass_filter, md.use_layer_group_filter]
    ...
    def init_tab_GREASE_PENCIL_THICKNESS(self):
        ...
        def fn_darklight(md):
            if ui_state == [ui_anim_data.library_state, md.use_uniform_thickness, md.use_weight_factor, md.vertex_group_name, md.use_layer_pass_filter, md.use_material_pass_filter, md.use_custom_curve, md.use_layer_group_filter]: return
            ui_state[:] = [ui_anim_data.library_state, md.use_uniform_thickness, md.use_weight_factor, md.vertex_group_name, md.use_layer_pass_filter, md.use_material_pass_filter, md.use_custom_curve, md.use_layer_group_filter]
    ...
    def init_tab_GREASE_PENCIL_COLOR(self):
        ...
        def fn_darklight(md):
            if ui_state == [ui_anim_data.library_state, md.use_layer_pass_filter, md.use_material_pass_filter, md.use_custom_curve, md.use_layer_group_filter]: return
            ui_state[:] = [ui_anim_data.library_state, md.use_layer_pass_filter, md.use_material_pass_filter, md.use_custom_curve, md.use_layer_group_filter]
    ...
    def init_tab_GREASE_PENCIL_TINT(self):
        ...
        def fn_darklight(md):
            if ui_state == [ui_anim_data.library_state, md.use_weight_as_factor, md.tint_mode, md.vertex_group_name, md.use_layer_pass_filter, md.use_material_pass_filter, md.use_custom_curve, md.use_layer_group_filter]: return
            ui_state[:] = [ui_anim_data.library_state, md.use_weight_as_factor, md.tint_mode, md.vertex_group_name, md.use_layer_pass_filter, md.use_material_pass_filter, md.use_custom_curve, md.use_layer_group_filter]
    ...
    def init_tab_GREASE_PENCIL_OPACITY(self):
        ...
        def fn_darklight(md):
            if ui_state == [ui_anim_data.library_state, md.color_mode, md.use_uniform_opacity, md.use_weight_as_factor, md.vertex_group_name, md.use_layer_pass_filter, md.use_material_pass_filter, md.use_custom_curve, md.use_layer_group_filter]: return
            ui_state[:] = [ui_anim_data.library_state, md.color_mode, md.use_uniform_opacity, md.use_weight_as_factor, md.vertex_group_name, md.use_layer_pass_filter, md.use_material_pass_filter, md.use_custom_curve, md.use_layer_group_filter]
    ...
    def bufn_MULTIRES_apply_base(self):
        ...
        #|
    def bufn_MULTIRES_conform_base(self):

        self.catch_bufn(
            lambda: bpy.ops.object.multires_base_apply(modifier=self.w.active_modifier.name, apply_heuristic=False),
            push_message = "MD Conform Base")
        #|
    ...
    def r_bufn_search(self, old_upd_callback, ui_animdatas, extra_buttons=None, search_data=None, search_confirm_callback=None):
        def bufn_search(button=None, use_old_data=False):
            ...
            def bufn_delete():
                ...
                if search_confirm_callback is not None:
                    search_confirm_callback()
                    self.upd_data()
                    Admin.REDRAW()
    ...
    def upd_data(self):
        ...
        if hasattr(ob, "animation_data"):
            ...
            object_fcurves = r_fcurves(animation_data)
...
RNA_MULTIRES_conform_base = RnaButton("MULTIRES_conform_base",
    name = "Conform Base",
    button_text = "Conform Base",
    description = "Modify the base mesh to conform to the displaced mesh")
