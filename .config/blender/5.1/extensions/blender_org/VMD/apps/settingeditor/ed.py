...
# <<< 1mp (VMD.api
api = VMD.api
r_ops_from_idname = api.r_ops_from_idname
r_rna_type_from_ops = api.r_rna_type_from_ops
r_py_cls_from_idname = api.r_py_cls_from_idname
rl_all_keymap_types = api.rl_all_keymap_types
# >>>
...
RNAS = {
    ...
    "keymap": {
        ...
        "modal": RnaSubtab("modal", "Modal Keymap", "Blender Modal Keymap.", "GpuImg_settings_keymap_ops"),
    },
}
...
class AreaBlockTabSettingEditor(AreaBlockTab):
    ...
    def init_tab_keymap_ops(self):
        ...
        self.items[:] = items
        #|
    def init_tab_keymap_modal(self):
        ...
        items = []
        bpy_ops = bpy.ops
        r_block_keymap_modal = self.r_block_keymap_modal

        o = r_block_keymap_modal(self, 'view3d.vmd_rotate')
        if o is not None:
            items.append(o)
        o = r_block_keymap_modal(self, 'view3d.vmd_dolly')
        if o is not None:
            items.append(o)
        o = r_block_keymap_modal(self, 'view3d.vmd_zoom')
        if o is not None:
            items.append(o)
    ...
    def r_block_pref_importexport(self, w):
        ...
        return out
        #|
    def r_block_keymap_modal(self, w, idname):
        o = r_ops_from_idname(idname)
        if o == None: return None

        ops_rna = r_rna_type_from_ops(o)
        o = BuFunction(None, RNA_edit,
            lambda: bufn_edit_modal_keymap(o, self.items[0].box_block, idname),
            False)
        o.init_bat = o.init_bat_anim

        out = BlockUtil(self, [AreaStringXYButton(None, self, ops_rna.description, self.width_wrap, FONT0)], o)
        out.use_anim_slot = False
        out.blf_title.text = idname
        return out
        #|
...
def bufn_edit_expression(button0=None, box_block=None):
    ...
    #|
def bufn_edit_modal_keymap(button0=None, box_block=None, idname=''):
    L, R, B, T = button0.box_button.r_LRBT()
    pp = P.modal_keymaps
    o = r_ops_from_idname(idname)
    at = r_py_cls_from_idname(idname).__name__

    def confirm_fn(s):
        error_start = 'Dictionary compilation error.\n'
        try:
            dic = json.loads(s)
            allow_keys = rl_all_keymap_types()
            allow_keys_set = set(allow_keys)

            for k, v in dic.items():
                if not isinstance(k, str):
                    return f'{error_start}Key "{k}" must be a string, may be missing double quotes.'
                if not isinstance(v, str):
                    return f'{error_start}Value of key "{k}": "{v}" must be a string, may be missing double quotes.'
                if v not in allow_keys_set:
                    allow_keys_str = ",\n".join([f'"{s}"' for s in allow_keys])
                    return f'{error_start}Value of key "{k}": "{v}" is not in the list of allowed key types.\n{allow_keys_str}'

        except json.JSONDecodeError as ex:
            return f'JSON dictionary compilation error.\n{ex}\n'

        setattr(pp, at, s)
        save_pref()
        return False

    def r_default_value(): return pp.bl_rna.properties[at].default

    DropDownText(button0, (box_block.L, R, T), getattr(pp, at), confirm_fn, r_default_value)
    #|
