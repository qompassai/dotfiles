...
# sub.sh
# Qompass AI FFMPEG Subliminal Generation Script
# Copyright (C) 2026 Qompass AI, All rights reserved
# ----------------------------------------
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(dirname "$SCRIPT_DIR")"
source "$ROOT_DIR/config/ffmpeg_subliminal.env"
IN="${1:?input video or gif required}"
OUT="${2:-subliminal.gif}"
WORKDIR="${WORKDIR:-/tmp/subliminal_work}"
mkdir -p "$WORKDIR"
PALETTE="$WORKDIR/palette.png"
FILTER_TEMPLATE="$ROOT_DIR/filters/ffmpeg_subliminal_text.ffscript"
