-- #################################################################
-- /qompassai/.config/buildcache/lua/ti_c6x_wrapper.lua
-- Qompass AI Ti C6x Wrapper
...
local function make_preprocessor_cmd(args, preprocessed_file)
	...
	for _, arg in ipairs(args) do
		...
		if
			(arg == '--compile_only')
			or starts_with(arg, '--output_file=')
			or starts_with(arg, '-pp')
			or starts_with(arg, '--preproc_')
		then
			drop_this_arg = true
		end
...
function resolve_args()
	...
	local new_args = {}
	for _, arg in ipairs(m_unresolved_args) do
		local response_file
...
function get_build_files()
	...
	for i = 2, #m_args do
		if starts_with(m_args[i], '--output_file=') then
			if found_object_file then
				error('Only a single target object file can be specified.')
			end
			files['object'] = m_args[i]:sub(15)
			found_object_file = true
		elseif starts_with(m_args[i], '-ppd=') or starts_with(m_args[i], '--preproc_dependency=') then
			if found_dep_file then
				error('Only a single dependency file can be specified.')
			end
			local eq = m_args[i]:find('=')
			files['dep'] = m_args[i]:sub(eq + 1)
			found_dep_file = true
		end
	end
...
function get_program_id()
	...
	local result = bcache.run({ m_args[1], '--help' })
...
function get_relevant_arguments()
	...
	for _, arg in ipairs(m_args) do
		if not skip_next_arg then
			...
			local is_unwanted_arg = (first_two_chars == '-I')
				or starts_with(arg, '--include')
				or starts_with(arg, '--preinclude=')
				or (first_two_chars == '-D')
				or starts_with(arg, '--define=')
				or starts_with(arg, '--c_file=')
				or starts_with(arg, '--cpp_file=')
				or starts_with(arg, '--output_file=')
...
function preprocess_source()
	...
	local preprocessor_args = make_preprocessor_cmd(m_args, preprocessed_file)
